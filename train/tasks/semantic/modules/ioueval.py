...
class iouEval:
  ...
  def getacc(self):
    ...
    prec_mean = total_tp / total
    return prec_mean  # returns "precission mean"
  
  def getAccuracy(self):
    tp, fp, fn = self.getStats()
    total_tp = tp.sum() + 1e-15
    total = tp[self.include].sum() + fp[self.include].sum() + fn[self.include].sum() + 1e-15
  ...
  def getRecall(self):
    tp, fp, fn = self.getStats()
    total_tp = tp.sum()
    total = tp[self.include].sum() + fn[self.include].sum() + 1e-15
    recall_mean = total_tp / total
    return recall_mean  # returns "recall  mean"
