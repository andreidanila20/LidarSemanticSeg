...
class C3Block(nn.Module):
  def __init__(self, c1, c2, n):
    ...
    #deoarece avem concatenarea al 3 lea block va avea si la intrare aceeasi dimensiune ca la iesire
    self.block3 = ConvBlock(2*c_, c2, 1, 1, 0)
    #catre bottleNeck trimitem inplanes=64, planes=[64,64]
    self.model  = nn.Sequential(*(BottleNeck(c_, c_) for _ in range(n)))
...
class Decoder(nn.Module):
  ...
  def _make_dec_layer(self, block, planes, just_upsample):
    ...
    if just_upsample or planes[2]==0:
      ...
      #  blocks
      layers.append(("C3", block(planes[0], planes[1], 3)))
